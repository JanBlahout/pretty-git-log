...
export async function fetchAndComputeStats(
	token: string,
	year: number,
): Promise<CodeStoryData> {
	"use cache";
	cacheLife("hours");
	...
	const activeDates = new Set(
		contributionDays.filter((d) => d.count > 0).map((d) => d.date),
	);
	...
	const langResults = await Promise.all(
		topReposByPush.map((r) => fetchRepoLanguages(token, r.full_name)),
	);
	...
	const monthlyActivity = Array.from({ length: 12 }, (_, i) => {
		...
		const monthPRs = prs.filter((pr) =>
			pr.created_at.startsWith(monthStr),
		).length;
	...
	const peakDay =
		DAY_NAMES[commitsByDay.indexOf(Math.max(...commitsByDay))] ?? "Monday";
	...
	const data: CodeStoryData = {
		...
		totalPRs: prs.length,
		totalPRsMerged,
		totalRepos: repos.filter((r) => r.pushed_at?.startsWith(String(year)))
			.length,
...
export async function fetchAndComputePublicStats(
	username: string,
	year: number,
): Promise<CodeStoryData> {
	"use cache";
