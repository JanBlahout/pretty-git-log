...
export function generatePersonalityTags(data: CodeStoryData): string[] {
	...
	const weekendCommits =
		(data.commitsByDay[0] ?? 0) + (data.commitsByDay[6] ?? 0);
	const weekdayCommits = data.commitsByDay
		.slice(1, 6)
		.reduce((a, b) => a + b, 0);
	...
	const hasFrontend = langNames.some((l) =>
		["typescript", "javascript", "css", "html", "vue", "svelte"].includes(l),
	);
	const hasBackend = langNames.some((l) =>
		[
			"python",
			"go",
			"rust",
...
export function generateNarrative(data: CodeStoryData): string {
	const peakMonthIdx = data.commitsByMonth.indexOf(
		Math.max(...data.commitsByMonth),
	);
	const months = [
		"January",
		"February",
	...
	const avgPRSize =
		data.totalPRs > 0 ? Math.round(data.totalCommits / data.totalPRs) : 0;
	const prSizeText =
		avgPRSize < 3 ? "small, focused" : avgPRSize < 8 ? "medium-sized" : "large";
	...
	if (data.totalPRs > 0) {
		lines.push(
			`You opened ${data.totalPRs} pull requests and favored ${prSizeText} PRs.`,
		);
	}
