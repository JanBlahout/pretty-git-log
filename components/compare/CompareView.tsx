...
export function CompareView({ usernames, initialData, currentYear }: Props) {
	const [dataByUser, setDataByUser] = useState<
		Record<string, Record<number, CodeStoryData>>
	>(() =>
		Object.fromEntries(
			usernames.map((u) => [
				u,
				initialData[u] ? { [currentYear]: initialData[u]! } : {},
			]),
		),
	);
	...
	const handleYearChange = async (year: number | "all") => {
		...
		try {
			const results = await Promise.allSettled(
				yearsToFetch.map(({ username, year: y }) =>
					fetchPublicYearStats(username, y),
				),
			);
			setDataByUser((prev) => {
				const next = { ...prev };
	...
	for (const username of usernames) {
		...
		if (activeYear === "all") {
			...
			statsPerUser[username] =
				allYearData.length > 0 ? aggregateStats(allYearData) : null;
		} else {
	...
	const winners: Record<keyof DisplayStats, string | null> = {} as Record<
		keyof DisplayStats,
		string | null
	>;
	for (const row of STAT_ROWS) {
		if (!row.numeric) {
			winners[row.key] = null;
			continue;
		}
		let best = -1;
	...
	return (
		<div className="min-h-screen bg-background">
			...
			<div
				...
				style={{
					background:
						"radial-gradient(ellipse 60% 50% at 50% 0%, rgba(139,92,246,0.10) 0%, transparent 70%)",
				}}
			>
				<div className="max-w-5xl mx-auto">
					<h1 className="text-3xl font-bold font-mono text-text-primary mb-8">
							...
							return (
								<div key={username} className="flex items-center gap-3">
									<div className="relative flex-shrink-0">
										<div
											className="absolute inset-0 rounded-full opacity-30 blur-[12px]"
			...
			<div className="max-w-5xl mx-auto px-6 pb-24 space-y-5">
				...
				<div className="rounded-2xl bg-surface border border-border overflow-hidden">
					...
					<div
						...
						style={{
							gridTemplateColumns: `1fr repeat(${usernames.length}, 1fr)`,
						}}
					>
						<div className="p-4" />
						{usernames.map((username, i) => (
