...
function computeAggregate(allData: CodeStoryData[]): CodeStoryData {
	...
	const peakHour = summedCommitsByHour.indexOf(
		Math.max(...summedCommitsByHour),
	);
...
export function DashboardView({
	...
	const sortedFetched = fetchedYears
		.sort((a, b) => b - a)
		.map((y) => dataByYear[y]);
	...
	const data =
		activeYear === "all"
			? computeAggregate(
					sortedFetched.length > 0 ? sortedFetched : [initialData],
				)
			: (dataByYear[activeYear] ?? initialData);

	const isLoading =
		activeYear !== "all" && loadingYears.has(activeYear as number);
	...
	const heroTitle =
		activeYear === "all"
			? `${data.name}'s full story`
			: `${data.name}'s ${activeYear}`;
	const navYear =
		activeYear === "all"
			? allFetched
				? "All time"
				: `${fetchedYears.length} years`
			: activeYear;
	...
	const keyStats = [
		{
			...
			sub: `${data.totalActiveDays} active days`,
		},
		{
			label: "Pull requests",
			value: data.totalPRs,
			sub: `${data.totalPRsMerged} merged`,
		},
		{
		...
		},
		{
			...
			sub: `${data.totalRepos} active repos`,
		},
	];
				...
				<div className="max-w-5xl mx-auto">
					...
					>
						<div className="relative flex-shrink-0">
							<div className="absolute inset-0 rounded-full opacity-40 bg-brand blur-[16px] scale-[85%]" />
							<Image
								src={data.avatarUrl}
								alt={data.login}
								width={96}
				...
				{isLoading ? (
					...
					<>
						...
						<Section title="Contribution activity">
							{activeYear === "all" ? (
								<MultiYearHeatmap
									years={sortedFetched.map((d) => ({
										year: d.year,
										days: d.contributionDays,
									}))}
								/>
							) : (
								<ContributionHeatmap
									...
									return (
										<YearComparison
											...
											prev={
												prevData
													? {
															commits: prevData.totalCommits,
															prs: prevData.totalPRs,
															repos: prevData.totalRepos,
														}
